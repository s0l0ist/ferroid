...
- **Snowflake Sync**: Benchmarks the hot path without yielding to the clock
- **Snowflake Async**: Uses 4096-ID batches, but may yield (sequence
  exhaustion/CAS failure) or await due to task scheduling, reducing throughput
- **ULID Sync & Async**: Uses the same 4096-ID batches. Due to random number
...
Tests were run on an M1 MacBook Pro 14", 32GB, 10 cores (8 performance, 2
efficiency).
